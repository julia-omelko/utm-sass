...
var OAuthAdapter = function (pConsumerSecret, pConsumerKey, pSignatureMethod,utm) {
    ...
    function authorizeUICallback(e) {
       // var response = e.source.evalJS('(p = document.getElementById("oauth_pin")) && p.innerHTML;');
        ...
        var oauthVerifierLocation = body.indexOf('oauth_verifier=');
        
        if( oauthVerifierLocation >0){
        		var oauthTokenLocation = body.indexOf('oauth_token=');
        		var oauthTokenLocationEnd = body.indexOf('&',oauthTokenLocation);
        		utm.TwitterOauthToken=body.substring(oauthTokenLocation+12,oauthTokenLocationEnd);
        		
        		var oauthVerifierLocation = body.indexOf('oauth_verifier=');
        		var oauthVerifierLocationEnd = body.indexOf('"',oauthVerifierLocation);
        		utm.TwitterOauthVerifier=body.substring(oauthVerifierLocation+15,oauthVerifierLocationEnd);      
        		
        		destroyAuthorizeUI();
            receivePinCallback();
        	
        }else{
        	//invalid login reshow twitter login page
        	   loadingView && loadingView.hide();
            loadingContainer && loadingContainer.hide();
            webView && webView.show();
        	
        }
      //  <meta http-equiv="refresh" content="0;url=http://dev.youthisme.com?oauth_token=Od8SJyKLZNpM1Tv1YhgQY06DEVCbmryr2jItkDnRc0&amp;oauth_verifier=RTazfVEJakXMI9rkj8CYxwPAmTVu70fb2SIJFGmk">
    ...
    this.getAccessToken = function (pUrl, callback) {
        ...
        var message = createMessage(pUrl);
        message.parameters.push(['oauth_token', utm.TwitterOauthToken]);
        message.parameters.push(['oauth_verifier', utm.TwitterOauthVerifier]);
            ...
            if (!adapter.isAuthorized()) {
                ...
                adapter.getRequestToken(urls.requestToken, function (evt) {
                    if (!evt.success) {
            			...
            			utm.TwitterToken=settings.consumerKey;
					utm.TwitterTokenSecret = settings.consumerSecret;
                callback && callback();
            }
        },
